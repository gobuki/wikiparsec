...
tests = test [
    "show term" ~: (show termRussian) ~?= "term [\"остынуть\",\"ru\"]", -- "{\"text\":\"остынуть\",\"language\":\"ru\"}",
    ...
    "extract ordered 1" ~: extractNumberedDefs "en" (OrderedList
                                                 [(Item "one"),
                                                  (OrderedList
                                                    [(Item "one point one"),
                                                     (Item "one point two")]),
                                                  (Item "two"),
                                                  (IndentedList [(Item "two example one")])]) ~?=
      [("def.1", "one"),
       ("def.1.1", "one point one"),
       ("def.1.2", "one point two"),
       ("def.2", "two"),
       ("def.2.ex.1", "two example one")],
